 ...
 */
export default function PredictionNews({ theme = "dark", query = "" }) {
  ...
  return (
    <div style={{ height: "100%", overflow: "auto", padding: "12px 8px", display: "flex", flexDirection: "column", gap: 10 }}>
      {searchUrl && (
        ...
        >
          Open “{query}” on Google News →
        </a>
      )}
      {items.length === 0 ? (
        <div style={{ padding: "12px 0", color: textSecondary, fontSize: 12 }}>
          No recent headlines. Use the button above to search on Google News.
        </div>
      ) : (
