...
/**
 * Google News search – "ticker crypto" | "ticker stock" | "ticker forex" | "ticker commodity" by topic.
 */
export default function CryptoNews({ theme = "dark", ticker = "ETH", isStock = false, symbol = "", topicLabel: topicLabelProp }) {
  const [items, setItems] = useState([]);
  const [searchUrl, setSearchUrl] = useState("");
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState(null);
  const topicLabel = topicLabelProp ?? (isStock ? "stock" : "crypto");
  // Build link on client so it always matches "X stock" / "X forex" / etc. regardless of API
  ...
  useEffect(() => {
    ...
    setLoading(true);
    setError(null);
    const q = encodeURIComponent((ticker || (topicLabel === "stock" ? "" : "ETH")).trim());
    const topicParam = topicLabel ? `&topic=${encodeURIComponent(topicLabel)}` : "";
    const symbolParam = topicLabel === "stock" && symbol ? `&symbol=${encodeURIComponent(symbol)}` : "";
