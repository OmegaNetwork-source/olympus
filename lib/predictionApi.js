...
// Empty string = same origin so Vite proxy (localhost:5173 → localhost:3001) is used in dev; set VITE_API_URL for prod.
...
/** Fetch prediction markets (server does multi-source aggregation). Use { light: true } for fast initial "popular" load. */
export async function fetchPredictionMarkets(tag = "", network = "solana", opts = {}) {
  try {
    let url = `${API}/markets?network=${encodeURIComponent(network)}`;
    if (tag) url += `&tag=${encodeURIComponent(tag)}`;
    if (opts.light) url += "&light=true";
